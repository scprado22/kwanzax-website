...
const typeBadgeStyles: Record<string, string> = {
  real: "bg-emerald-500/20 text-emerald-100 border-emerald-400/30",
  illustrative: "bg-amber-500/20 text-amber-100 border-amber-400/30",
  internal: "bg-white/15 text-white/80 border-white/20",
};
    ...
    <div className="bg-white">
      ...
      <section className="py-20">
        <div className="max-w-7xl mx-auto px-6">
          ...
          >
            {p.cases.map((project) => {
              const typeLabel = p.caseTypeLabel[project.type as keyof typeof p.caseTypeLabel];
              const badgeStyle = typeBadgeStyles[project.type] ?? typeBadgeStyles.internal;
              return (
                <motion.article
                  key={project.slug}
                  variants={fadeUp}
                ...
                >
                  {/* Card header */}
                  <div className={`relative h-48 bg-gradient-to-br ${caseGradients[project.slug]} overflow-hidden`}>
                  ...
                  <div className="p-7 space-y-5">
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-sm text-gray-600 mt-1.5 leading-relaxed">{project.problem}</p>
                      </div>
                      <div>
                        <span className="text-xs font-semibold text-gray-400 uppercase tracking-widest">
                          {p.solutionLabel}
                        </span>
                        <p className="text-sm text-gray-600 mt-1.5 leading-relaxed">{project.solution}</p>
                      </div>
                      <div>
                        <span className="text-xs font-semibold text-gray-400 uppercase tracking-widest">
                          {p.resultLabel}
                        </span>
                        <p className="text-sm text-gray-700 font-medium mt-1.5 leading-relaxed">{project.result}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 pt-1">
                      {caseUrls[project.slug] && (
                        ...
                        >
                          {p.viewWebsite} <ExternalLink size={13} />
                        </a>
                      )}
                      <Link
                        to="/contactos"
                      ...
                      >
                        {p.similarProject} <ArrowRight size={14} />
                      </Link>
                    </div>
                  </div>
                </motion.article>
      ...
      <section className="bg-[#03409C] py-24 relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-6 text-center">
          ...
          >
            ...
            >
              {p.ctaButton} <ArrowRight size={18} />
            </Link>
          </motion.div>
        </div>
      </section>
